...
from model import Linear, EmbeddingModule, SwiGLU, scaled_dot_product_attention, RotaryPositionalEmbedding,\
      MultiHeadSelfAttention, TransformerBlock, RMSNorm, TransformerLM
...
def run_linear(
    ...
    y1 = in_features.matmul(weights.T)
    y2 = torch.einsum("...i,oi->...o", in_features, weights)
    y3 = in_features @ weights.T
...
def run_multihead_self_attention(
    ...
    """
    attn = MultiHeadSelfAttention(d_model, num_heads, seq_len=in_features.shape[-2], apply_rope=False)
    with torch.no_grad():
        attn.q_proj.weight.copy_(q_proj_weight)
        attn.k_proj.weight.copy_(k_proj_weight)
        attn.v_proj.weight.copy_(v_proj_weight)
        attn.out_proj.weight.copy_(o_proj_weight)
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    attn = MultiHeadSelfAttention(d_model, num_heads, seq_len=max_seq_len, apply_rope=True, theta=theta)
    with torch.no_grad():
        attn.q_proj.weight.copy_(q_proj_weight)
        attn.k_proj.weight.copy_(k_proj_weight)
        attn.v_proj.weight.copy_(v_proj_weight)
        attn.out_proj.weight.copy_(o_proj_weight)
    return attn.forward(in_features, token_positions)   
...
def run_transformer_block(
    ...
    """
    block = TransformerBlock(d_model, num_heads, seq_len=max_seq_len, d_ff=d_ff, apply_rope=True, theta=theta)
    with torch.no_grad():
...
def run_transformer_lm(
    ...
    """
    model = TransformerLM(vocab_size, context_length, d_model, num_layers, num_heads, d_ff, apply_rope=True, theta=rope_theta)
    with torch.no_grad():
